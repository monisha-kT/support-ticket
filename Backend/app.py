...
# app.config['MAIL_USERNAME'] = 'monishak2020csbs@gmail.com'
# app.config['MAIL_PASSWORD'] = 'nhyr zeht uwmz lqeq'
# app.config['MAIL_DEFAULT_SENDER'] = 'monishak2020csbs@gmail.com'
...
#     reassign_reason = db.Column(db.Text, nullable=True) 
#     reassigned_to = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
#     last_message_at = db.Column(db.DateTime, nullable=True)
#     subject = db.Column(db.String(50), nullable=False)
#     auto_generated_key=db.Column(db.String(50), nullable=True)
...
# #                 auto_generated_key=f"TICKET-{datetime.now(IST).strftime('%Y%m%d')}-{current_user_id}"
# #             )
...
# #             'auto_generated_key': ticket.auto_generated_key
# #         }, namespace='/admin') 
...
# #             'auto_generated_key': t.auto_generated_key
...
#                 ticket_id=ticket.id,
#                 sender_id=None,
#                 message=f"Ticket created by {user.first_name} {user.last_name}.",
#                 timestamp=datetime.now(IST),
#                 is_system=True
#             )
...
#             try:
#                 if not user.email:
#                     logger.warning(f"No email found for user ID {current_user_id} for ticket {ticket.auto_generated_key}")
#                 else:
#                     msg = Message(
#                         subject=f"Ticket {ticket.auto_generated_key} Created",
...
# Ticket ID: {ticket.auto_generated_key}
# Subject: {ticket.subject}
# Category: {ticket.category}
# Priority: {ticket.priority}
# Created At: {ticket.created_at.strftime('%d %b %Y %I:%M %p IST')}

# Description:
# {ticket.description}

# You can track your ticket status in the support dashboard.
...
#                     )
#                     mail.send(msg)
#                     logger.info(f"Ticket creation email sent to {user.email} for ticket {ticket.auto_generated_key}")
#             except Exception as e:
#                 logger.error(f"Failed to send ticket creation email to {user.email}: {str(e)}")
...
#         # Check permissions
#         user = User.query.get(current_user_id)
#         if user.role not in ['admin', 'support_lead']:
#             return jsonify({"error": "Unauthorized"}), 403

#         # Check if the user to reassign to exists and is a member
#         new_assignee = User.query.filter_by(id=reassign_to, role='member').first()
#         if not new_assignee:
#             return jsonify({"error": "Valid member to reassign not found"}), 404

#         # Store previous assignee before updating
#         previous_assignee = ticket.assigned_to

#         # Update the ticket fields
#         ticket.assigned_to = reassign_to
#         ticket.status = 'assigned'  # Change status to 'assigned' after reassignment
#         ticket.reassigned_to = previous_assignee  # Store who it was reassigned from
...
#                 'assigned_to': ticket.assigned_to,
#                 'reassigned_to': ticket.reassigned_to,
#                 'reassign_reason': ticket.reassign_reason,
#                 'category': ticket.category,
#                 'priority': ticket.priority,
#                 'subject': ticket.subject,
#                 'description': ticket.description,
...
#             'member': {
#                 'id': new_assignee.id,
#                 'first_name': new_assignee.first_name,
#                 'last_name': new_assignee.last_name,
...
#             'assigned_to': ticket.assigned_to,
#             'status': ticket.status,
#             'member_name': f"{new_assignee.first_name} {new_assignee.last_name}"
#         }, room='members')
...
#     except Exception as e:
#         db.session.rollback()
#         print(f"Error in reassigning ticket: {str(e)}")
#         return jsonify({"error": "Failed to reassign ticket"}), 500
...
#         ticket = Ticket.query.get_or_404(ticket_id)
#         if ticket.status not in ['open', 'assigned', 'reassigned']:
#             return jsonify({'error': 'Cannot reassign closed or rejected ticket'}), 400
...
#             pass
#         elif user.role == 'member':
#             # Member can only close tickets assigned to them
#             if ticket.assigned_to != int(current_user_id):
#                 return jsonify({'error': 'Unauthorized: Not assigned to this ticket'}), 403
#         elif user.role == 'user':
#             # User can only close tickets they created
#             if str(ticket.user_id) != str(current_user_id):
#                 return jsonify({'error': 'Unauthorized: You did not create this ticket'}), 403
...
#             ticket.reassigned_to = reassign_to
#             ticket.assigned_to = reassign_to
#             ticket.status = 'reassigned'  # Keep as assigned if reassigned
#         else:
#             ticket.status = 'closed'
#             ticket.reassigned_to = None
...
app.config['MAIL_USE_TLS'] = True
app.config['MAIL_USERNAME'] = 'monishak2020csbs@gmail.com'
app.config['MAIL_PASSWORD'] = 'nhyr zeht uwmz lqeq'  # Ensure this is a Gmail App Password
app.config['MAIL_DEFAULT_SENDER'] = 'monishak2020csbs@gmail.com'
...
def reassign_ticket(ticket_id):
    try:
        ...
        data = request.get_json()
        reassign_to = data.get('reassign_to')
        reason = data.get('reason', 'Reassigned by member')
        ...
        system_message = ChatMessage(
            ticket_id=ticket_id,
            sender_id=None,
            message=f"Ticket reassigned to {reassign_user.first_name} {reassign_user.last_name}. Reason: {reason}",
            timestamp=datetime.now(IST),
            is_system=True
        )
        db.session.add(system_message)
        ...
        try:
            if ticket.user_id:
                ...
                if ticket_user and ticket_user.email:
                    msg = Message(
                        subject=f"Ticket {ticket.auto_generated_key} Reassigned",
                        recipients=[ticket_user.email],
                        body=f"""
Dear {ticket_user.first_name} {ticket_user.last_name},

Your ticket has been reassigned to {reassign_user.first_name} {reassign_user.last_name}.
...
Support Team
                    ...
                    mail.send(msg)
                    logger.info(f"Reassignment email sent to {ticket_user.email} for ticket {ticket.auto_generated_key}")
        except Exception as e:
        ...
        if ticket.status == 'closed':
            try:
                ...
                if not ticket_user or not ticket_user.email:
                    logger.warning(f"No email found for user ID {ticket.user_id} for ticket {ticket.auto_generated_key}")
                else:
                    chat_messages = ChatMessage.query.filter_by(ticket_id=ticket_id).order_by(ChatMessage.timestamp.asc()).all()
                    conversation = "\n".join([
                        f"[{msg.timestamp.strftime('%d %b %Y %I:%M %p IST')}] "
                        f"{'System' if msg.is_system else (User.query.get(msg.sender_id).first_name + ' ' + User.query.get(msg.sender_id).last_name if msg.sender_id else 'Unknown')}: "
                        f"{msg.message}"
                        for msg in chat_messages
                    ])
                    msg = Message(
                        subject=f"Ticket {ticket.auto_generated_key} Closed",
                        recipients=[ticket_user.email],
                        body=f"""
Dear {ticket_user.first_name} {ticket_user.last_name},
...
Closed At: {ticket.closed_at.strftime('%d %b %Y %I:%M %p IST')}
...
Support Team
                    ...
                    mail.send(msg)
                    logger.info(f"Ticket closure email sent to {ticket_user.email} for ticket {ticket.auto_generated_key}")
            except Exception as e:
                logger.error(f"Failed to send ticket closure email to {ticket_user.email}: {str(e)}")
        ...
        return jsonify({'message': f"Ticket {'reassigned_to' if reassign_to else 'closed'} successfully"}), 200
    except Exception as e:
        logger.error(f"Error closing ticket: {str(e)}")
        db.session.rollback()
...
def reopen_ticket(ticket_id):
    try:
        ...
        socketio.emit('ticket_reopened', {'ticket_id': ticket_id}, room=str(ticket_id))
        return jsonify({'message': 'Ticket reopened successfully'}), 200
    except Exception as e:
        logger.error(f"Error reopening ticket: {str(e)}")
        db.session.rollback()
...
def get_last_chat_message(ticket_id):
    try:
        ...
        logger.error(f"Error fetching last message: {str(e)}")
        return jsonify({'error': str(e)}), 500
    ...
    with app.app_context():
        ...
        for ticket in tickets:
            ...
            try:
                ticket_user = User.query.get(ticket.user_id)
                if ticket_user and ticket_user.email:
                    chat_messages = ChatMessage.query.filter_by(ticket_id=ticket.id).order_by(ChatMessage.timestamp.asc()).all()
                    conversation = "\n".join([
                        f"[{msg.timestamp.strftime('%d %b %Y %I:%M %p IST')}] "
                        f"{'System' if msg.is_system else (User.query.get(msg.sender_id).first_name + ' ' + User.query.get(msg.sender_id).last_name if msg.sender_id else 'Unknown')}: "
                        f"{msg.message}"
...
Your ticket has been closed due to 24 hours of inactivity.

Ticket ID: {ticket.auto_generated_key}
Subject: {ticket.subject}
Closure Reason: {ticket.closure_reason}
Closed At: {ticket.closed_at.strftime('%d %b %Y %I:%M %p IST')}
...
Support Team
                    ...
                    mail.send(msg)
                    logger.info(f"Inactivity closure email sent to {ticket_user.email} for ticket {ticket.auto_generated_key}")
            except Exception as e:
                logger.error(f"Failed to send inactivity closure email: {str(e)}")
