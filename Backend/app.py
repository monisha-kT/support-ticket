...
class ChatMessage(db.Model):
    ...
    sender_name=db.Column(db.String,db.ForeignKey('users.first_name'),nullable=True)
...
def send_async_email(app, msg):
    with app.app_context():
        try:
            mail.send(msg)
            print("Mail sent successfully (thread).")  # or logger.info
        except Exception as e:
            print(f"Mail send failed (thread): {e}")   # or logger.error
...
def handle_message(data):
    try:
        ...
        if not sender:
            logger.error(f"Sender not found: {data['sender_id']}")
            return
...
def reopen_ticket(ticket_id):
    try:
        ...
        if ticket.status not in ['closed', 'rejected']:
            return jsonify({'error': 'Ticket cannot be reopened unless it is closed or rejected'}), 400


        # 🛠 Extract reason and assign_to
        data = request.get_json()
        reason = data.get('reason', 'Reopened by system')
        assign_to = data.get('assign_to')  # can be None or int

        # Update ticket
        ticket.status = 'assigned' if assign_to else 'open'
...
def get_chat_messages(ticket_id):
    try:
        ...
        # Join ChatMessage with User table to get sender_name
        messages = (
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
    Thread(target=start_inactivity_checker, daemon=True).start()
